    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>Make changes to your post. Allowed since it hasn&apos;t been verified yet.</DialogDescription>
        </DialogHeader>
...
export function RequestModificationModal({
  ...
  const handleSubmit = async () => {
    ...
    setIsSubmitting(true)
    await communityClient.requestModification(post.id, currentUserId, { description: requestedChanges }, reason)
    setIsSubmitting(false)
    onClose()
  }
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>Your post is verified, so changes must be approved by an admin.</DialogDescription>
        </DialogHeader>
        <div className="grid gap-4 py-4">
          <div className="grid gap-2">
...
export function RequestRemovalModal({
  ...
  const handleSubmit = async () => {
    if (!reason) return
    setIsSubmitting(true)
    await communityClient.requestRemoval(post.id, currentUserId, reason)
    setIsSubmitting(false)
    onClose()
  }
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>Your post is verified, so removal must be approved by an admin.</DialogDescription>
        </DialogHeader>
        <div className="grid gap-4 py-4">
          <div className="grid gap-2">
            ...
            <textarea
              value={reason}
              onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setReason(e.target.value)}
              placeholder="Why do you want to delete this post?"
              className="min-h-[100px] w-full rounded-md border border-input bg-background px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-ring resize-none"
            />
          </div>
        </div>
